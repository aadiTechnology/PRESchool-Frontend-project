...
export async function fetchStudents(divisionId: number): Promise<Student[]> {
  ...
  const res = await fetch(`http://localhost:8000/api/v1/auth/attendance/students?divisionId=${divisionId}`, {
    headers: {
...
export async function fetchAttendance(divisionId: number, date: string): Promise<AttendanceSummary[]> {
  const token = localStorage.getItem('token');
  const res = await fetch(`http://localhost:8000/api/v1/auth/attendance?divisionId=${divisionId}&date=${date}`, {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
    },
...
export async function markAttendance(divisionId: number, date: string, attendance: AttendanceRecord[]): Promise<void> {
  const token = localStorage.getItem('token');
  const res = await fetch(`http://localhost:8000/api/v1/auth/attendance/mark`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
